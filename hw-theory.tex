...
\begin{enumerate}
\item Покажите, что для $a,b : Set\ S$ выполнено $a = b$ тогда и только тогда, когда они состоят из одинаковых элементов ($\forall x:S.x \in a \rightarrow x \in b$).
\item Аксиома пары: если $p,q : Set\ S$, то $\{p,q\} : P(Set S)$.
\item Аксиома объединения: если $\forall i.p(i) \in Set\ S$, то $\cup_i p(i) : Set\ S$.
\item Аксиома выделения: если $s : Set\ S$ и $p : S \rightarrow \text{Prop}$, то $\{ x\in s | p(x)\} : Set S$.
\item Выполнена ли аксиома выбора для таких множеств?
\item Пусть $A,B : Set\ S$. Верно ли, что $\forall p: Set\ S \rightarrow \text{Prop}.p(A) \leftrightarrow p(B)$ эквивалентно $A=B$?
Возможно ли доказать $p(A)=p(B)$ при $A=B$ (например, при помощи аксиомы унивалентности)?
...
\item Задачи на перечисление элементов и неравенства.
\begin{enumerate}
\item Покажите, что $\forall a \in \mathbb{N}. \forall x \in \mathbb{N}. a < x \& x < a + 3 \rightarrow x = a +1 \vee x =a +2$
\item Определим множественное равенство:
\begin{verbatim}
\func P (N : Nat) (x : Nat) : \Prop \elim N
  | 0 => 0 = x
  | suc N => (P N x) || (suc N = x)
\end{verbatim}

\item Покажите, что $x < 4$ тогда и только тогда, когда $P\ 3\ x$.
\item В условии прошлой задачи покажите, что $x < a+1$ тогда и только тогда, когда $P\ a\ x$.
\end{enumerate}

\item Докажите, что число 239 не является гёделевым номером никакой строки. Напомним, $n$ --- гёделев номер строки $s_1\dots s_n$, если 
$n = 2^{\ulcorner s_1 \urcorner}\cdot 3^{\ulcorner s_2 \urcorner} \dots$. Лишние множители присутствовать в номере не должны.

\item Ещё несколько фактов про импликативные решётки (баллы начисляются за пронумерованное буквой задание, выполненное целиком):
\begin{enumerate}
...
\item (15 баллов) Покажите, что в любом связном графе не меньше $n-1$ дуги.
В дереве (связный граф без циклов) $n-1$ дуга.
В дереве каждые две вершины соединены единственным путём.
